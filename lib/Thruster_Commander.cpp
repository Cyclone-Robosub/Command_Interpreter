...
Thruster_Commander::Thruster_Commander(std::string filename)
{
	...
	volume_center = mass_center;
	volume_center(0, 2) += 0.1;
	thruster_positions = thruster_set_3D::Zero();
	...
	wrench_matrix = wrench_matrix_transposed.transpose();
	max_thruster_force = config["max_thruster_force"].as<float>();
	min_thruster_force = config["min_thruster_force"].as<float>();
	position = six_axis::Zero();
	velocity = six_axis::Zero();
	acceleration = six_axis::Zero();
						 ...
						 int closestRowIndex)
{
	...
	{ // Case when closest row/force is smaller than desired force
	...
	{ // Case when closest row/force value is larger than desired force
		if (numericData[(closestRowIndex)][0] == 0 && numericData[(closestRowIndex - 1)][0] == 0)
		{
			closestRowIndex--;
		}
...
thruster_set Thruster_Commander::thrust_compute_fx_fy_mz(float x_force, float y_force, float z_torque)
{
	...
	coefficients << -8.72533307e-02f, 1.08905877e-01f, 2.62141732e-01f, -1.22850123e+00f, -9.84251969e-01f, 4.77297359e-17f,
...
float Thruster_Commander::accel_time_x(float v_i, float v)
{
	...
	if (forward)
	{
		force_per_thruster = min_thruster_force;
	}
	else
	{
		force_per_thruster = max_thruster_force;
	}
